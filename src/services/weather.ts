...
export async function getWeather7d(cityName: string) {
    const config = await readConfig();
    const apiKey = config.weather?.apiKey || process.env.HEWEATHER_KEY;
    if (!apiKey) {
        console.error('HeWeather API key not configured. Configure it in the settings or add HEWEATHER_KEY to your .env file.');
    ...
    try {
        ...
        const [nowResponse, forecastResponse] = await Promise.all([
            axios.get(`https://devapi.qweather.com/v7/weather/now?location=${locationId}&key=${apiKey}`),
            axios.get(`https://devapi.qweather.com/v7/weather/7d?location=${locationId}&key=${apiKey}`)
        ]);
        ...
        return {
            ...
            forecast: forecast.map((day: any) => ({
                ...
                description: day.textDay,
                icon: day.iconDay,
                iconId: day.iconDay.replace(/\D/g, '')
            }))
        };
    } catch (error) {
        if (axios.isAxiosError(error)) {
            console.error(`Error fetching 7-day weather for ${cityName}:`, error.response?.data || error.message);
        } else if (error instanceof Error) {
            console.error(`Error fetching 7-day weather for ${cityName}:`, error.message);
        } else {
            console.error(`An unknown error occurred while fetching 7-day weather for ${cityName}:`, error);
        }
        throw error;
    }
}